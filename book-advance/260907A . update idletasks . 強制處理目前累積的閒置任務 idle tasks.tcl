...
Kiro
update idletasks 是 Tcl/Tk 的指令，用來強制處理目前累積的「閒置任務 (idle tasks)」,
最常見的用途是立即刷新 UI 畫面，而不等到程式回到事件迴圈。

背景：Tk 的畫面更新機制
Tk 不會在你每次改變 widget 時立刻重畫，
而是把「重繪、重新排版」這類工作排進 idle queue（閒置佇列），
等到程式空閒（回到事件迴圈、沒有其他事在跑）時才一次處理。

問題來了：如果你在一個長時間執行的迴圈裡更新 UI，
程式一直忙著跑迴圈、沒空回到事件迴圈，畫面就會卡住不動（凍結），直到迴圈跑完才一次更新。
...
update 的危險在於：
它會處理使用者的點擊事件，若使用者在你的迴圈執行中又按了「開始測試」按鈕，該按鈕的 callback 會在迴圈中間被觸發，造成程式邏輯錯亂、甚至遞迴呼叫自己而 crash。
...
剛建立 widget 後想立刻取得它的實際尺寸
（winfo width 在畫面還沒 layout 時會回傳 1，先 update idletasks 強制排版就能拿到正確值）
...
更好的替代方案
不過，若是非阻塞式設計（像你 iperf 工具用 fileevent + after 讓程式自然回到事件迴圈），
其實不太需要 update idletasks，因為畫面本來就會在每次 idle 時自動刷新。
update idletasks 主要是「不得已在長迴圈中卡住事件迴圈」時的補救手段。
